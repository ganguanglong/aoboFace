package com.facepp.library.mvp.view.activity.permission;
...
/**
 * 项目名：   ABFaceSys
 * 包名：     com.facepp.library.mvp.view.activity.permission
 * 创建者：   Arsenalong
...
public class PermissionActivity  extends BaseActivity implements EasyPermissions.PermissionCallbacks{
    ...
    private void requestPerm() {
        if (EasyPermissions.hasPermissions(this, StaticClass.perms)) {
            startActivity(new Intent(this,DetectActivity.class));
            finish();
        } else {
            EasyPermissions.requestPermissions(this, "授予所必须的权限",
                    RC, StaticClass.perms);
        }
    }
